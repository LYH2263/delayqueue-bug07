...
const StageTag7 = "brokerutil-stage7"
...
func (s *Stage7) MergeJSON(a, b map[string]any) (map[string]any, error) {
	ba, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
...
func (s *Stage7) ClampDuration(d, min, max time.Duration) time.Duration {
	if d < min {
		return min
	}
	if d > max {
...
func (s *Stage7) TruncateRunes(sv string, n int) string {
...
func (s *Stage7) Digest(parts ...string) string {
	sum := sha256.New()
	for _, p := range parts {
...
func (s *Stage7) JoinNonEmpty(sep string, ss ...string) string {
