...
const StageTag11 = "brokerutil-stage11"
...
func (s *Stage11) MergeJSON(a, b map[string]any) (map[string]any, error) {
	ba, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
...
func (s *Stage11) ClampDuration(d, min, max time.Duration) time.Duration {
	if d < min {
		return min
	}
	if d > max {
...
func (s *Stage11) TruncateRunes(sv string, n int) string {
...
func (s *Stage11) Digest(parts ...string) string {
	sum := sha256.New()
	for _, p := range parts {
...
func (s *Stage11) JoinNonEmpty(sep string, ss ...string) string {
