...
const StageTag4 = "brokerutil-stage4"
...
func (s *Stage4) HashLabel(label string) string {
	h := fnv.New32a()
	_, _ = h.Write([]byte(label))
	_, _ = h.Write([]byte(strconv.Itoa(s.n)))
	return fmt.Sprintf("tag-%08x", h.Sum32())
}
func (s *Stage4) SortKeys(m map[string]int) []string {
...
func (s *Stage4) MergeJSON(a, b map[string]any) (map[string]any, error) {
	ba, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
...
func (s *Stage4) ClampDuration(d, min, max time.Duration) time.Duration {
	if d < min {
		return min
	}
	if d > max {
...
func (s *Stage4) TruncateRunes(sv string, n int) string {
...
func (s *Stage4) Digest(parts ...string) string {
	sum := sha256.New()
	for _, p := range parts {
...
func (s *Stage4) JoinNonEmpty(sep string, ss ...string) string {
