...
const StageTag8 = "brokerutil-stage8"
...
func (s *Stage8) MergeJSON(a, b map[string]any) (map[string]any, error) {
	ba, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
...
func (s *Stage8) ClampDuration(d, min, max time.Duration) time.Duration {
	if d < min {
		return min
	}
	if d > max {
...
func (s *Stage8) TruncateRunes(sv string, n int) string {
...
func (s *Stage8) Digest(parts ...string) string {
	sum := sha256.New()
	for _, p := range parts {
...
func (s *Stage8) JoinNonEmpty(sep string, ss ...string) string {
