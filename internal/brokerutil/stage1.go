...
const StageTag1 = "brokerutil-stage1"
...
func (s *Stage1) HashLabel(label string) string {
	h := fnv.New32a()
	_, _ = h.Write([]byte(label))
	_, _ = h.Write([]byte(strconv.Itoa(s.n)))
	return fmt.Sprintf("tag-%08x", h.Sum32())
}
func (s *Stage1) SortKeys(m map[string]int) []string {
...
func (s *Stage1) MergeJSON(a, b map[string]any) (map[string]any, error) {
	ba, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
...
func (s *Stage1) ClampDuration(d, min, max time.Duration) time.Duration {
	if d < min {
		return min
	}
	if d > max {
...
func (s *Stage1) TruncateRunes(sv string, n int) string {
...
func (s *Stage1) Digest(parts ...string) string {
	sum := sha256.New()
	for _, p := range parts {
...
func (s *Stage1) JoinNonEmpty(sep string, ss ...string) string {
